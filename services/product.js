...
const productDetailProjection ={images:1,name:1,price:1,colors:1,available:1,sku:1,brand:1,rate:1,reviews:1,_id:1};
  
exports.getAllProducts = async () =>{
    const products = await productModel.find({},productsProjection);
    if(products)
       return respContent(true,"total : " + products.length,products);
...
exports.getProductDetail = async (id) =>{
    ...
    if(product)
       return respContent(true,"",product);
    throw Error("Sorry, no products matched your search.");
}
...
exports.search = async (searchKey, category, company, colors, price, shipping,sort) =>{
    ...
    const products = await productModel.find({$and: conditions},productsProjection).sort(sort).limit(50);
    if(products)
       return respContent(true,"total : " + products.length,products);
    throw Error("Sorry, no products matched your search.");
}